...
import { Trophy, ChevronRight, Star, ExternalLink } from 'lucide-react';
...
export function Leaderboard() {
  ...
  // Function to open leaderboard in a new window
  const openStandalonePage = () => {
    // Serialize the current leaderboard data
    const leaderboardData = JSON.stringify(teams);
    
    // Save to localStorage to share with the popup window
    localStorage.setItem('algoforge_leaderboard_data', leaderboardData);

    // Open a new window with the standalone leaderboard
    window.open('/leaderboard-standalone', '_blank', 'width=800,height=800');
  };
    ...
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 p-6">
        ...
        <div className="text-center mb-12 mt-4">
          <h1 className="text-4xl md:text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#007bff] to-[#00bfff] mb-2 tracking-tighter">
            AlgoForge
          </h1>
          ...
          {/* Button to open standalone page */}
          <button 
            onClick={openStandalonePage}
            className="mt-4 px-4 py-2 bg-[#000d1a] border border-[#007bff] rounded-md text-[#00bfff] hover:bg-[#007bff]/20 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#007bff] flex items-center mx-auto"
          >
        ...
        {error && (
          <div className="max-w-4xl mx-auto p-4 bg-red-900/40 border border-red-500 rounded-lg text-center">
            <p className="text-red-300">{error}</p>
            <button 
              onClick={() => leaderboardService.getLeaderboardData().then(setTeams)}
              className="mt-2 px-4 py-2 bg-red-500/30 hover:bg-red-500/50 text-white rounded-md"
            >
              Retry
            </button>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-12 gap-4 mb-4 px-4 py-3 rounded-t-lg bg-gradient-to-r from-[#000d1a] to-[#001a33] text-white font-medium border-b border-[#007bff]/50">
              ...
              <div className="col-span-3 flex items-center justify-end">
                <p className="font-bold text-[#00bfff]">SCORE</p>
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-12 gap-4 px-4 py-3 items-center">
                    ...
                    <div className="col-span-2 flex items-center justify-center">
                      <div className="relative flex items-center justify-center">
                        {team.rank && team.rank <= 3 ? (
                          <Trophy className={`w-5 h-5 ${getTrophyColor(team.rank)}`} />
                        ) : (
                          <span className="text-xl font-mono text-gray-300">{team.rank}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="col-span-7 flex items-center">
                      <div className="flex flex-col">
                        <div className="flex items-center">
                          <ChevronRight className="w-4 h-4 text-[#00bfff] mr-2" />
                          <span className={`font-medium ${team.rank && team.rank <= 3 ? 'text-white text-lg' : 'text-gray-200'}`}>
                            {team.teamName}
                          </span>
                        </div>
                        {team.projectName && (
                          <span className="text-xs text-gray-400 ml-6 mt-1">
